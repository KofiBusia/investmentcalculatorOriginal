"""
Blast assessment invitation email to all registered SiteUsers.
Run once: venv\Scripts\python.exe blast_assessment_email.py
"""

import sys
import time
from app import app, db, mail, SiteUser
          ...
          <table width="100%" cellpadding="0" cellspacing="0" style="margin-bottom:24px;border-radius:10px;overflow:hidden;border:1px solid #e5e7eb;">
            ...
            <tr style="background:#f0fdf4;">
              <td style="padding:14px 20px;">
                <p style="margin:0;font-size:13px;font-weight:700;color:#166534;">&#9654; Haven't taken the assessment yet?</p>
                <p style="margin:4px 0 0;font-size:13px;color:#374151;">All study materials and practice questions are available at the link below — take the test today.</p>
              </td>
            </tr>
          </table>
          ...
          <table width="100%" cellpadding="0" cellspacing="0" style="margin-bottom:24px;border-radius:10px;overflow:hidden;background:#fffbeb;border:1px solid #fcd34d;">
            <tr>
              <td style="padding:16px 20px;">
                ...
                <p style="margin:8px 0 0;font-size:12px;color:#9ca3af;">After payment, visit the link below and register for your chosen track.</p>
              </td>
            </tr>
          </table>

          <p style="margin:0 0 8px;font-size:14px;color:#374151;line-height:1.6;">
            &#128218; All <strong>study materials and practice questions</strong> are also available at the same link — use them to prepare before you sit the assessment.
          </p>
...
Haven't taken the assessment yet?
  All study materials and practice questions are available at the link below — take the test today.
...
HOW TO PAY:
  ...
  Then visit the link below to complete your registration.

TAKE THE ASSESSMENT & STUDY MATERIALS:
...
TEST_EMAIL = "kyeikofi@gmail.com"

def run(test_only=False):
    with app.app_context():
        if test_only:
            users = SiteUser.query.filter_by(email=TEST_EMAIL).all()
            print(f"TEST MODE — sending only to {TEST_EMAIL}")
        else:
            users = SiteUser.query.filter_by(is_active=True).all()
        total = len(users)
        print(f"Found {total} user(s). Starting email blast...")
        ...
        for u in users:
            try:
                ...
                msg.html = html_body(u.full_name or u.email)
                msg.body = plain_body(u.full_name or u.email)
                mail.send(msg)
                sent += 1
                print(f"  [{sent}/{total}] Sent → {u.email}")
                time.sleep(0.5)   # gentle rate-limit
            except Exception as e:
                failed.append(u.email)
                print(f"  FAILED → {u.email}: {e}", file=sys.stderr)
